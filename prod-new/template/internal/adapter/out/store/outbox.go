...
type Entry struct {
	...
	State          EntryState
	Attempts       int
	IdempotencyKey string // set once Publish's retry closure has minted one
}
...
func (o *Outbox) Journal(effect domain.Effect) (string, error) {
	...
	defer o.mu.Unlock()
	id := o.ids()
	o.entries[id] = &Entry{ID: id, Effect: effect, State: StateIntent}
	return id, nil
}
...
func (o *Outbox) Publish(ctx context.Context, entryID string) error {
	...
	for attempt := 1; attempt <= o.maxAttempts; attempt++ {
		// The idempotency key is minted INSIDE this closure body, lazily,
		// on the FIRST iteration only -- every subsequent retry of this
		// same logical Publish call reuses it. A key minted OUTSIDE the
		// loop (by the caller, or by Journal itself) would be
		// indistinguishable from this in the happy path but would silently
		// break the moment a caller retried an ENTIRE Publish call after an
		// ambiguous outer failure (e.g. this process crashed mid-retry and
		// a new process calls Publish again for the same entryID) -- THAT
		// case is exactly what a fresh key-per-Publish-invocation, minted
		// inside THIS call's own closure, protects: each independent
		// logical delivery attempt (across process restarts) gets its own
		// key, while retries WITHIN one attempt share it.
		if idempotencyKey == "" {
			idempotencyKey = o.ids()
		}
